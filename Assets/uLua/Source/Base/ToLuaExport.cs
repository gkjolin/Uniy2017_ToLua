/*    
 * Copyright (广州纷享游艺设备有限公司-研发视频组) 2014.9 , 蒙占志 (Zhanzhi Meng) topameng@gmail.com
 * All rights reserved.
...
public static class ToLuaExport 
{
    ...
    public static void Generate(params string[] param)
    {
        ...
        if (type.IsEnum)
        {
            ...
            GenEnumTranslator();
            sb.AppendLine("}\r\n");
            SaveFile(Const.LuaWrapPath + wrapClassName + "Wrap.cs");
            return;
        }
    ...
    static bool NeedDotweenNamespace()//给导出dotween的文件新增dotween的命名空间
    {
        switch (libClassName)
        {
    ...
    {        
        ...
        sb.AppendLine("}");        
        SaveFile(Const.LuaBasePath + "Base/DelegateFactory.cs");
